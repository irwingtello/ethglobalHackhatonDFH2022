(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('@uauth/common'), require('crypto'), require('jsonwebtoken'), require('jwk-to-pem')) :
  typeof define === 'function' && define.amd ? define(['exports', '@uauth/common', 'crypto', 'jsonwebtoken', 'jwk-to-pem'], factory) :
  (global = global || self, factory(global.UAuthNode = {}, global.common, global.crypto, global.jsonwebtoken, global.jwkToPem));
})(this, (function (exports, common, crypto, jwt, jwkToPem) {
  ...
  const sha256 = s => {
    return crypto.createHash('sha256').update(new TextEncoder().encode(s)).digest().buffer;
  };
  const toBase64 = buf => {
  ...
  const verifyIdToken = async (jwks_uri, jwks, id_token, nonce) => {
    ...
    if (jwks_uri) {
      secretOrPublicKey = (header, cb) => {
        fetch(jwks_uri).then(res => res.json()).then(({
          ...
          if (key) {
            cb(null, jwkToPem__default["default"](key));
            return;
          }
    ...
    const idToken = await new Promise((resolve, reject) => {
      jwt__default["default"].verify(id_token, secretOrPublicKey, options, (error, decoded) => {
        if (error) {
          reject(error);
        }
  ...
  class Client {
    constructor(options) {
      ...
      const self = this;
      this.issuerResolver = new common.DefaultIssuerResolver({
        webfingerResolver: new common.DefaultWebFingerResolver({
          ipfsResolver: new common.DefaultIPFSResolver((...args) => (this.options.createIpfsUrl || common.DefaultIPFSResolver.defaultCreateUrl)(...args)),
          domainResolver: {
            records(domain, keys) {
